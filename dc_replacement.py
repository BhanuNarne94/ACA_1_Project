...
class DuelingClock:
    ...
    def determineSetGroup(self, set_num):
        ...
        if (set_num >= utilities.DC_GROUP1_LOWER) and (set_num < utilities.DC_GROUP1_UPPER):
            self.policy_select -= 1
            return "CLOCK"
        elif (set_num >= utilities.DC_GROUP1_UPPER) and (set_num < utilities.DC_GROUP2_UPPER):
            self.policy_select += 1
            return "SCAN"
        elif (set_num >= utilities.DC_GROUP2_UPPER) and (set_num < self.sets):
            if msb == 1:
                return "CLOCK"
            else:
                return "SCAN"
